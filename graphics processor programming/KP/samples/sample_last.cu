...
int main(int argc, char* argv[]) {
	...
	for (int k = 0; k < frame_num; k++) {
        ...
        if (on_gpu) {
            kernel_render<<<dim3(16, 32), dim3(16, 32)>>>(pc, pv, w_smooth, h_smooth, angle, dev_data, dev_figures, figures.size(), light_source, light_shade);
            CSC(cudaGetLastError());

            kernel_ssaa<<<dim3(16, 32), dim3(16, 32)>>>(dev_data, dev_short_data, w, h, sqrt_ray_num);
            CSC(cudaGetLastError());
